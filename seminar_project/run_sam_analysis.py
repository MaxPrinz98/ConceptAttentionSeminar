...
def process_directory(set_path, model, device, force=False):
    ...
    results = model(
        ...
        device=device,
        verbose=False,
        retina_masks=True,  # Uses high-resolution processing for better edges
        conf=0.10,  # Drastically lower the confidence threshold
        iou=0.2,  # Lower the overlap threshold
    )
    ...
    # --- ADD SYNTHETIC BACKGROUND SEGMENT ---
    # Calculate union of all current masks
    all_masks_union = np.zeros(masks.shape[1:], dtype=bool)
    for i in range(masks.shape[0]):
        all_masks_union = np.logical_or(all_masks_union, masks[i] > 0.5)

    # Create the background gap mask (everything SAM missed)
    background_mask = ~all_masks_union

    # Only add it if the gap actually contains pixels
    if background_mask.sum() > 0:
        # Append the background mask to the existing masks array
        # Reshape to [1, H, W] to concatenate
        background_mask_expanded = background_mask.astype(np.float32)[np.newaxis, ...]
        masks = np.concatenate([masks, background_mask_expanded], axis=0)
        print("    Added 1 synthetic segment to cover SAM gaps.")
    # ----------------------------------------
    ...
    # Now this will naturally report 100% because of the synthetic segment
    all_masks_union_final = np.zeros(masks.shape[1:], dtype=bool)
    for i in range(num_segments):
        all_masks_union_final = np.logical_or(all_masks_union_final, masks[i] > 0.5)

    overall_coverage_percent = (
        np.sum(all_masks_union_final) / all_masks_union_final.size
    ) * 100
    print(f"    Overall SAM coverage: {overall_coverage_percent:.2f}%")

    # Save coverage gap visualization
    # This should now result in a clean image with almost no red gaps
    coverage_vis = img_np.copy()
    coverage_vis[~all_masks_union_final] = [255, 0, 0]  # Red for gaps
