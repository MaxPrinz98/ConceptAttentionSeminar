...
<body>
    ...
    def render_failure_cards(candidates, color_hex):
        if not candidates:
        ...
        for cand in candidates[:10]:
            ...
            # Use masked attention image (not raw heatmap)
            ...
            # Load SAM coverage percentage
            coverage_percent = 0.0
            summary_path = cp / "sam_analysis" / "segments_summary.json"
            if summary_path.exists():
                try:
                    with open(summary_path, "r") as f:
                        summary_data = json.load(f)
    ...
    failure_halluc_html = render_failure_cards(hallucination_candidates, "#f85149")
    failure_missed_html = render_failure_cards(
        missed_localization_candidates, "#d29922"
    )
    ...
    html_content += f"""
            ...
            <div id="failure" class="tab-content">
                ...
                <h3 style="color: #f85149; border-bottom: 1px solid #f8514933; padding-bottom: 10px;">Hallucinations (showing worst {min(10, len(hallucination_candidates))} of {len(hallucination_candidates)})</h3>
                {failure_halluc_html}
